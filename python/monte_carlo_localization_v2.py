...
class ParticleFilter(object):
    ...
    def handle_observation(self, laser_scan, dt):
        ...
        for particle in self.particles:
            new_particle = self.predict_particle_odometry(particle)
            if not new_particle:
                new_particle = Particle(particle.id, particle.x, particle.y, particle.theta)
            error = self.get_prediction_error_squared(laser_scan, new_particle)
            errors.append(-error)
        b = max(errors)
        errors = [exp(error - b) for error in errors]
        sum_weights = sum(errors)
        self.weights = [error/sum_weights for error in errors]
    ...
    def resample(self):
        ...
        # self.particles = []
        # N_eff = 1 / sum([weight**2 for weight in self.weights])
        # if N_eff < 50:
        #     return 
        new_particles = []
        sample_r = np.random.uniform(0, 1)
        N = self.num_particles
        c = self.weights[0]
        idx = 0
        for i in range(N):
            sample_u = sample_r + (i-1)/N
            while sample_u > c:
                idx = (idx + 1) % N
                c += self.weights[idx]
            p = self.particles[idx]
            new_particles.append(Particle(p.id, p.x, p.y, p.theta))
    ...
    def simulate_laser_scan_for_particle(self, x, y, yaw_in_map, angles, min_range, max_range):
        ...
        for angle in angles:
            ...
            while curr_range <= max_range:
                curr_x, curr_y = x + curr_range*cos(abs_angle), y + curr_range*sin(abs_angle)
                if curr_x > self.xmax or curr_x < self.xmin or curr_y > self.ymax or curr_y < self.ymin:
                    break
                if not self.grid_bin[self.metric_to_grid_coords(curr_x, curr_y)].all():
                    break
    ...
    def get_prediction_error_squared(self, laser_scan_msg, particle):
        ...
        norm_error = 0
        for i in range(len(predict_ranges)):
            norm_error += (actual_ranges[i] - predict_ranges[i])**2
        return norm_error
    ...
    def predict_particle_odometry(self, particle):
        ...
        return Particle(particle.id, particle.x, particle.y, particle.theta)
